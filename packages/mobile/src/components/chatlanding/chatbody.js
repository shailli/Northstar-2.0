...
const renderMedia = (item, userId) => {
    if (item.mediaType === 'camera') {
        ...
        return (
            <TouchableOpacity
                style={[styles.displayFile, checkUser(item, userId)]}
                onPress={() => console.log('Icon included')}
        ...
        function checkUser(data, userId) {
            if (data.userId === userId) return styles.ownText;
...
const downloadFile = link => {
    ...
    firebase
        .storage()
        .refFromURL(link)
        .downloadFile(`${firebase.storage.Native.EXTERNAL_STORAGE_DIRECTORY_PATH}/Download/${fileName}`)
        .then(data => {
            console.log(data);
        })
        .catch(err => {
